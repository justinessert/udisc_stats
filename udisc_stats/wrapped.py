...
def print_score_pct(player_name, year, df, prev_df, score_name):
    ...
    print(f"{player_name}'s {score_name.lower()} percentage in {year} was {score_pct}% ({diff_score_pct:+}%)")
...
def stats_wrapped(player_name, year, df):
    ...
    player_score_df = score_df[
        (score_df["PlayerName"] == player_name) &
        (score_df["Year"] == year)
    ]
    player_prev_score_df = score_df[
        (score_df["PlayerName"] == player_name) &
        (score_df["Year"] == prev_year)
    ]

    player_score_agg_df = player_score_df.groupby(['PlayerName', 'Year', 'ScoreName']).sum().reset_index()
    n_holes = player_score_agg_df["Frequency"].sum()
    player_score_agg_df["Frequency"] = (player_score_agg_df["Frequency"] / n_holes * 100).astype(int)

    player_prev_score_agg_df = player_prev_score_df.groupby(['PlayerName', 'Year', 'ScoreName']).sum().reset_index()
    n_prev_holes = player_prev_score_agg_df["Frequency"].sum()
    player_prev_score_agg_df["Frequency"] = (player_prev_score_agg_df["Frequency"] / n_prev_holes * 100).astype(int)

    diff_holes = int((n_holes - n_prev_holes) / n_prev_holes * 100)

    print(f"{player_name} played {n_holes} holes in {year} ({diff_holes:+}%)")
    print("")
    print_score_pct(player_name, year, player_score_agg_df, player_prev_score_agg_df, "Birdie")
    print_score_pct(player_name, year, player_score_agg_df, player_prev_score_agg_df, "Par")
    print_score_pct(player_name, year, player_score_agg_df, player_prev_score_agg_df, "Bogie")
